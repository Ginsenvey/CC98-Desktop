    ...
    public sealed partial class LatexBlock : UserControl 
    {
        ...
        protected override Windows.Foundation.Size MeasureOverride(Windows.Foundation.Size availableSize)
        {
            if (string.IsNullOrEmpty(_latex))
            {
                ...
                // 测量Content
                if (this.Content is FrameworkElement _content)
                {
                    _content.Measure(emptySize);
                }
            ...
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    float extraBottomSpace = lineHeight * 0.2f; // 增加20%的底部空间
            ...
            // 计算总尺寸（内容 + Padding）
            double totalWidth = _contentWidth + Padding.Left + Padding.Right;
            double totalHeight = _contentHeight + Padding.Top + Padding.Bottom;
        ...
        protected override Windows.Foundation.Size ArrangeOverride(Windows.Foundation.Size finalSize)
        {
            ...
            if (this.Content is FrameworkElement content)
            {
                content.Arrange(new Windows.Foundation.Rect(0, 0, finalSize.Width, finalSize.Height));
            }
        ...
        private void OnPaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            ...
            // 获取画布实际尺寸
            float canvasWidth = (float)_canvas.ActualWidth;
            float canvasHeight = (float)_canvas.ActualHeight;
            ...
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    // 绘制公式 - 给底部增加额外空间
                    float baselineOffset = Math.Abs(rect.Y); // rect.Y 是负值，表示基线以上的高度
                    float extraBottomSpace = rect.Height * 0.2f; // 增加20%的底部空间
                    _painter.Draw(canvas, new SKPoint(x, currentY + rect.Height + extraBottomSpace));
        ...
        public string DebugInfo
        {
            get
            {
                int lineCount = _latex?.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).Length ?? 0;
